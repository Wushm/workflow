...
from workflow.settings import vob_branch_status as _vob_branch_status
from workflow.settings import vob_branch_release_number as _vob_branch_release_number
...
def vob_branch_status():
    json = simplejson.dumps(_vob_branch_status, ensure_ascii = False) 
    return json

def vob_branch_release_number():
    json = simplejson.dumps(_vob_branch_release_number, ensure_ascii = False) 
    return json
    
approvers = register.simple_tag(approvers)
approver_name = register.simple_tag(approver_name)
vob_branch_status = register.simple_tag(vob_branch_status)
vob_branch_release_number = register.simple_tag(vob_branch_release_number)
