...
def _can_approve(u, cclock):
    ...
    def get_v_from_approvers(k):
        from workflow.VobConfig.views import branch_is_lock_and_superAdmin
        if k == 'None':
            return []
        if k:
            #对于已经被锁住的分支，只有分支管理员有权限通过审核
            isLock,vob_admin_name = branch_is_lock_and_superAdmin(k)
            if(isLock):
                return (vob_admin_name,)
            return approvers[k]
        return []
...
def all_branch_can_approve(u,cclock):
    result_message = ''
    if cclock.approve_result:
        return False,result_message
    message = None
    username = u.username
    def branch_can_approve(username,branch):
        if branch == 'None':
            return None
        from workflow.VobConfig.views import branch_is_lock_and_superAdmin
        if branch:
            isLock,vob_admin_name = branch_is_lock_and_superAdmin(branch)
            if isLock:
                if username == 'admin' or username == vob_admin_name:
                    return None
                vob_admin = User.objects.get(username = vob_admin_name)
                admin_cn_name = vob_admin.first_name
                message = '%s has been locked,lease contact the %s\r\n' %(branch,admin_cn_name)
                return message
            else:
                return None
        
    message = branch_can_approve(username,cclock.vob_branch_1)
    if message:
        result_message = result_message + message
    message = branch_can_approve(username,cclock.vob_branch_2)
    if message:
        result_message = result_message + message
    message = branch_can_approve(username,cclock.vob_branch_3)
    if message:
        result_message = result_message + message
    message = branch_can_approve(username,cclock.vob_branch_4)
    if message:
        result_message = result_message + message
    message = branch_can_approve(username,cclock.vob_branch_5)
    if message:
        result_message = result_message + message
    message = branch_can_approve(username,cclock.vob_branch_6)
    if message:
        result_message = result_message + message
...
def _format_bugs(bugs):
    ...
    try:
        if bug_list[0].find("BAN") > -1:
            current_db = "HZ_BANudb"
        elif bug_list[0].find("SBB") > -1:
            current_db = "SBBudb"
    ...
    for b in bug_list:
        ...
        if not mr:
            ...
            query="SELECT id, headline, state_name FROM dbo.Dev_Projects_MR_Info where id='%s'" % b
            ms_cur.execute(query)
            bug=ms_cur.fetchone()
            #bug=(b,"",)
            if not bug:
                query="SELECT id, headline, state_name FROM dbo.Fld_Projects_MR_Info where id='%s'" % b
                ms_cur.execute(query)
                bug=ms_cur.fetchone()            
            if not bug:
                query="SELECT id, headline, state FROM ProductSchema.dev_defect where id='%s'" % b
                ms_cur.execute(query)
                bug=ms_cur.fetchone()            
            if not bug:
                #如果MR不存在...
                if not command_mr(b):
                    ...
                    bug = b.split()
                    mr = MR(mr_id=bug[0],mr_headline=bug[1],mr_state='none')
                    mr.save()
                    form_bugs.append(mr.mr_id)
                    continue
            s=""
            mr=MR(mr_id=bug[0], mr_headline=bug[1].decode("GBK").encode("UTF-8"), mr_state=bug[2])#GBK2UTF8
            mr.save()
        form_bugs.append(mr.mr_id)
    ms_conn.close()
    return error, form_bugs
...
def _get_all_approver_mails(cclock):
    ...
    def get_all_approver_mails(vob_branch):
        emails = {}
        if vob_branch:
            t_usernames = approvers[vob_branch]
...
def approve(request, id, action=''):
    ...
    if request.POST or action:
        ...
        if not action:
            action = data['action']
        if 'o' == action:
            can_approve,message = all_branch_can_approve(request.user,cclock)
            if not can_approve:
